...
{
	//if the value is Null, calls Stack_Error function
	if (stack == NULL)
	{
		Stack_Error(SERR_INIT);
	}
	//else the stack initialization occurs by setting the value to -1
	else
	{
		stack->topIndex = -1;
	}
...
{
	//if the stack is empty(the value is set to - 1) it returns TRUE else returns FALSE
...
{
	//returns FALSE if the top index is not equal to STACK_SIZE (the stack is not full), else returns TRUE
...
{
	//if the stack is empty, calls an error function
	if (Stack_IsEmpty(stack))
	{
		Stack_Error(SERR_TOP);
	}
	//else saves the char from the top of the stack and saves it to dataPtr
	else
	{
...
{
	//checks if the stack is empty, if not deletes the top char by decrementing the top pointer
	if (!Stack_IsEmpty(stack))
	{
		stack->topIndex--;
	}
...
{
	//if the stack is not full, increments the top index and adds a new given char
	if (!Stack_IsFull(stack))
	{
		(stack->topIndex)++;
		stack->array[stack->topIndex] = data;
	}
	//else calls an error function
	else
	{
